document.addEventListener('DOMContentLoaded', () => {
    ...
    // 1. Creamos un unico cartel (tooltip) para toda la pagina.
    const tooltip = document.createElement('div');
    tooltip.className = 'paw-chart-tooltip';
    document.body.appendChild(tooltip);

    // 2. Buscamos todos los contenedores donde hay que dibujar un grafico
    ...
    charts.forEach(chartContainer => {
        try {
            ...
            if(total === 0) {
                chartContainer.innerHTML = '<p class="paw-chart-empty">No hay datos registrados a�n.</p>';
                return;
            }
            ...
            data.forEach((item, index) => {
                ...
                // MATEMATICA: Usamos Seno y Coseno para calcular las coordenadas X e Y donde empieza y termina la porci�n
            ...
            slices.forEach(slice => {
                slice.addEventListener('mouseenter', (e) => {
                    ...
                    tooltip.appendChild(document.createTextNode(`${item.valor} (${percentageText})`));

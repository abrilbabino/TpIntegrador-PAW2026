        ...
        <form id="form-editar-mascota" method="POST" action="/mascota/editar/guardar" enctype="multipart/form-data" class="perfil-datos form-editar-mascota" novalidate>
            ...
            <ul class="perfil-datos-grid">
                <li class="dato-item">
                    ...
                    <input type="text" name="nombre" value="<?= htmlspecialchars($nombreActual, ENT_QUOTES, 'UTF-8') ?>" class="dato-valor-input input-value <?= isset($errores['nombre']) ? 'input-invalido' : '' ?>" minlength="2" maxlength="60" pattern="^[A-Za-zÁÉÍÓÚáéíóúÑñ\s'-]+$" required data-trim-required="true" title="Solo se permiten letras, espacios, apóstrofes y guiones." <?= isset($errores['nombre']) ? 'data-server-error="' . htmlspecialchars($errores['nombre'], ENT_QUOTES, 'UTF-8') . '"' : '' ?>>
                </li>

                <li class="dato-item">
                    <span class="dato-label">Especie *</span>
                    <select name="especie" class="dato-valor-input input-value <?= isset($errores['especie']) ? 'input-invalido' : '' ?>" required <?= isset($errores['especie']) ? 'data-server-error="' . htmlspecialchars($errores['especie'], ENT_QUOTES, 'UTF-8') . '"' : '' ?>>
                ...
                <li class="dato-item">
                    ...
                    <select name="tamanio" class="dato-valor-input input-value <?= isset($errores['tamanio']) ? 'input-invalido' : '' ?>" required <?= isset($errores['tamanio']) ? 'data-server-error="' . htmlspecialchars($errores['tamanio'], ENT_QUOTES, 'UTF-8') . '"' : '' ?>>
                ...
                <li class="dato-item">
                    ...
                    <select name="temperamento" class="dato-valor-input input-value <?= isset($errores['temperamento']) ? 'input-invalido' : '' ?>" required <?= isset($errores['temperamento']) ? 'data-server-error="' . htmlspecialchars($errores['temperamento'], ENT_QUOTES, 'UTF-8') . '"' : '' ?>>
                ...
                <li class="dato-item">
                    ...
                    <label>
                        <input type="radio" name="sexo" value="macho" required <?= ($sexoActual === 'macho') ? 'checked' : '' ?> <?= isset($errores['sexo']) ? 'data-server-error="' . htmlspecialchars($errores['sexo'], ENT_QUOTES, 'UTF-8') . '"' : '' ?>>
                        Macho
                    </label>
                    <label>
                ...
                <li class="dato-item">
                    ...
                    <label>
                        <input type="radio" name="esterilizado" value="si" required <?= ($esterilizadoActual === 'si') ? 'checked' : '' ?> <?= isset($errores['esterilizado']) ? 'data-server-error="' . htmlspecialchars($errores['esterilizado'], ENT_QUOTES, 'UTF-8') . '"' : '' ?>>
                        Sí
                    </label>
                    <label>
                ...
                <li class="dato-item">
                    ...
                    <textarea name="descripcion_mascota" class="dato-valor-input input-value <?= isset($errores['descripcion_mascota']) ? 'input-invalido' : '' ?>" maxlength="500" <?= isset($errores['descripcion_mascota']) ? 'data-server-error="' . htmlspecialchars($errores['descripcion_mascota'], ENT_QUOTES, 'UTF-8') . '"' : '' ?>><?= htmlspecialchars($descripcionActual, ENT_QUOTES, 'UTF-8') ?></textarea>
                </li>

                <li class="dato-item">
                    <span class="dato-label">Fecha de nacimiento</span>
                    <input type="date" name="fecha_nacimiento" value="<?= htmlspecialchars($fechaNacimientoActual, ENT_QUOTES, 'UTF-8') ?>" max="<?= date('Y-m-d') ?>" data-no-future="true" data-future-message="La fecha de nacimiento no puede ser futura." class="dato-valor-input input-value <?= isset($errores['fecha_nacimiento']) ? 'input-invalido' : '' ?>" <?= isset($errores['fecha_nacimiento']) ? 'data-server-error="' . htmlspecialchars($errores['fecha_nacimiento'], ENT_QUOTES, 'UTF-8') . '"' : '' ?>>
                </li>

                <li class="dato-item">
                    <span class="dato-label">Foto Principal</span>
                    <input type="file" name="foto" accept="image/*" class="dato-valor-input input-value <?= isset($errores['foto']) ? 'input-invalido' : '' ?>" data-max-file-size="2097152" data-max-file-message="La imagen no puede superar 2 MB." <?= isset($errores['foto']) ? 'data-server-error="' . htmlspecialchars($errores['foto'], ENT_QUOTES, 'UTF-8') . '"' : '' ?>>
                </li>
            </ul>
        ...
        <section class="galeria-multimedia">
            ...
            <ul class="galeria-grid">
                <?php foreach ($fotos as $foto): ?>
                    <li class="foto-item">
                        <figure>
                            <img src="/<?= htmlspecialchars($foto->url, ENT_QUOTES, 'UTF-8') ?>" alt="Foto extra de la mascota" class="editar-mascota-foto">
                            <figcaption>
